    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col gap-1">
                <h1 className="text-2xl font-bold text-(--text-primary)">Gate & POS</h1>
            ...
            <div className="bg-linear-to-br from-indigo-500 to-purple-600 rounded-xl shadow-lg p-6 text-white overflow-hidden relative">
                <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                    ...
                    <div className="bg-(--surface)/10 backdrop-blur-sm rounded-lg p-4 border border-white/20">
            ...
            <div className="grid lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-4">
                    ...
                    {events.length === 0 ? (
                        <div className="bg-(--surface) rounded-xl p-12 text-center border border-(--border) shadow-sm">
                            ...
                            <h3 className="text-lg font-medium text-(--text-primary) mb-2">Belum Ada Event Aktif</h3>
                            ...
                            <button
                                ...
                                className="px-5 py-2.5 bg-(--accent-primary) text-white rounded-lg font-medium hover:opacity-90 transition-colors shadow-sm inline-flex items-center gap-2"
                            >
                                Buat Event Baru
                            </button>
                        </div>
                        ...
                        <div className="space-y-3">
                            {events.map((event) => (
                                <button
                                    ...
                                    className="w-full bg-(--surface) rounded-xl p-4 border border-(--border) shadow-sm hover:shadow-md transition-all group text-left flex items-center gap-4"
                                >
                                    {event.posterImage ? (
                                        <img
                                    ...
                                    <div className="shrink-0 flex items-center justify-center w-8 h-8 rounded-full bg-(--surface-hover) group-hover:bg-(--accent-primary)/10 transition-colors">
                                        <ChevronRight className="h-5 w-5 text-(--text-muted) group-hover:text-(--accent-primary) transition-colors" />
                                    </div>
                                </button>
                            ))}
                        </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-(--surface) rounded-xl p-6 border border-(--border) shadow-sm h-fit">
                        <ol className="relative border-l border-(--border) ml-3 space-y-6 my-2">
                            <li className="ml-6">
                                ...
                                <h4 className="font-medium text-(--text-primary) text-sm">Pilih Event</h4>
                                <p className="text-sm text-(--text-muted) mt-1">Pilih event aktif dan generate PIN akses gate.</p>
                            </li>
                            <li className="ml-6">
                                <span className="absolute flex items-center justify-center w-6 h-6 bg-indigo-100 rounded-full -left-3 ring-4 ring-white">
                                    <span className="text-xs font-bold text-(--accent-primary)">2</span>
                                </span>
                                <h4 className="font-medium text-(--text-primary) text-sm">Bagikan PIN</h4>
                                <p className="text-sm text-(--text-muted) mt-1">Berikan PIN kepada staff yang bertugas di pintu masuk.</p>
                            </li>
                            <li className="ml-6">
                                <span className="absolute flex items-center justify-center w-6 h-6 bg-indigo-100 rounded-full -left-3 ring-4 ring-white">
                                    <span className="text-xs font-bold text-(--accent-primary)">3</span>
                                </span>
                                <h4 className="font-medium text-(--text-primary) text-sm">Akses Gate</h4>
                                <p className="text-sm text-(--text-muted) mt-1">Staff mengakses <span className="font-mono bg-(--bg-secondary) px-1 py-0.5 rounded text-xs">/gate/access</span> dan login dengan PIN.</p>
                            </li>
                            <li className="ml-6">
                                <span className="absolute flex items-center justify-center w-6 h-6 bg-indigo-100 rounded-full -left-3 ring-4 ring-white">
                                    <span className="text-xs font-bold text-(--accent-primary)">4</span>
                                </span>
                                <h4 className="font-medium text-(--text-primary) text-sm">Scan & Jual</h4>
                                <p className="text-sm text-(--text-muted) mt-1">Mulai scan tiket pengunjung atau jual tiket on-the-spot.</p>
                            </li>
                        </ol>
                    </div>
                </div>
