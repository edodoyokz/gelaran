...
export default function GateManagementPage() {
    ...
    if (error || !data) {
        return (
            <div className="p-8 text-center">
                ...
                <p className="text-(--text-secondary) mt-2">{error || "Data tidak ditemukan"}</p>
                <button 
    ...
    return (
        <div className="min-h-screen bg-(--surface-hover)/50 pb-24">
            <div className="bg-(--surface) border-b border-(--border) sticky top-0 z-10 px-4 py-4 md:px-8">
                <div className="max-w-5xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-xl font-bold text-foreground">Manajemen Gate & POS</h1>
                            <p className="text-sm text-(--text-muted)">Kelola akses masuk dan penjualan on-site</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <main className="max-w-5xl mx-auto px-4 py-8 md:px-8 space-y-8">
                <div className="bg-(--surface) rounded-xl shadow-sm border border-(--border) p-6 flex items-start gap-6">
                    <div className="w-24 h-32 bg-(--bg-secondary) rounded-lg shrink-0 overflow-hidden">
                        {event.posterImage ? (
                ...
                <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-(--surface) rounded-xl shadow-sm border border-indigo-100 overflow-hidden">
                        <div className="p-4 border-b border-indigo-100 bg-(--accent-primary)/10/50 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 bg-indigo-100 rounded-lg flex items-center justify-center">
                                    <Camera className="w-5 h-5 text-(--accent-primary)" />
                                </div>
                                <div>
                                    <h3 className="font-semibold text-foreground">Gate Scanner</h3>
                                    <p className="text-xs text-(--text-muted)">Akses untuk check-in peserta</p>
                                </div>
                            </div>
                        ...
                        <div className="p-6 space-y-4">
                            <div className="space-y-3">
                                <div className="flex items-center justify-between">
                                    <label className="text-sm font-medium text-(--text-secondary)">Batas Perangkat</label>
                                ...
                                <input 
                                    ...
                                    className="w-full px-3 py-2 border border-(--border) rounded-lg focus:ring-2 focus:ring-(--accent-primary) focus:border-(--accent-primary) outline-none transition-all"
                                />
                            </div>
                            ...
                            <button
                                ...
                                className="w-full py-2.5 bg-(--accent-primary) hover:opacity-90 text-white font-medium rounded-lg shadow-sm shadow-indigo-200 transition-all flex items-center justify-center gap-2 disabled:opacity-70"
                            >
                                {isGenerating ? (
                            ...
                            {gateSession?.activeDevices && gateSession.activeDevices.length > 0 && (
                                <div className="pt-4 border-t border-(--border)">
                                    <p className="text-xs font-medium text-(--text-muted) uppercase tracking-wider mb-2">Perangkat Aktif</p>
                                    <div className="space-y-2">
                                        {gateSession.activeDevices.map((device) => (
                                            <div key={device.id} className="flex items-center gap-3 p-2 rounded-lg bg-(--accent-primary)/10">
                                                <div className="w-8 h-8 bg-indigo-100 rounded-full flex items-center justify-center">
                                                    <Smartphone className="w-4 h-4 text-(--accent-primary)" />
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-sm font-medium text-foreground truncate">{device.staffName}</p>
                                                    <p className="text-xs text-(--text-muted) truncate">{device.userAgent}</p>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                    ...
                    {/* POS Kasir Session */}
                    <div className="bg-(--surface) rounded-xl shadow-sm border border-emerald-100 overflow-hidden">
                        <div className="p-4 border-b border-emerald-100 bg-emerald-50/50 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="font-semibold text-foreground">POS Kasir</h3>
                                    <p className="text-xs text-(--text-muted)">Akses untuk penjualan on-site</p>
                                </div>
                            </div>
                            {posSession?.isActive && (
                                <span className="px-2.5 py-1 bg-emerald-100 text-emerald-700 text-xs font-medium rounded-full">
                        ...
                        <div className="p-6 space-y-4">
                            <div className="space-y-3">
                                ...
                                <input 
                                    ...
                                    className="w-full px-3 py-2 border border-(--border) rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all"
                                />
                            </div>
            ...
            <div className="fixed bottom-0 left-0 right-0 p-4 bg-(--surface) border-t border-(--border) z-20">
                <div className="max-w-5xl mx-auto flex items-center justify-between gap-4">
                    <div className="hidden md:block">
                        <p className="text-sm font-medium text-foreground">Mode Organizer</p>
                        <p className="text-xs text-(--text-muted)">Buka Gate/POS tanpa login PIN</p>
                    </div>
                    <div className="flex-1 md:flex-none flex gap-3">
                        <Link
                            href={`/gate?direct=true&eventId=${eventId}`}
                            className="flex-1 md:flex-none md:w-auto px-5 py-3 bg-(--accent-primary) hover:opacity-90 text-white font-medium rounded-xl shadow-lg shadow-indigo-200 transition-all flex items-center justify-center gap-2"
                        >
            ...
            {showStaffModal && generatedPin && eventSlug && generatedSessionType && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-(--surface) rounded-2xl shadow-2xl max-w-md w-full overflow-hidden">
                        ...
                        <div className="p-6 space-y-4">
                            <div className="space-y-2">
                                <label className="text-xs font-medium text-(--text-muted) uppercase tracking-wider">Kode Event</label>
                                <div className="flex items-center gap-2">
                                    <div className="flex-1 bg-(--surface-hover) border border-(--border) rounded-lg px-4 py-3 font-mono text-foreground">
                                        {eventSlug}
                                    </div>
                                    <button onClick={copyEventSlug} className="p-3 text-(--text-muted) hover:text-(--accent-primary) hover:bg-(--accent-primary)/10 rounded-lg border border-(--border) transition-colors">
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-medium text-(--text-muted) uppercase tracking-wider">PIN Akses</label>
                                <div className="flex items-center gap-2">
                                    <div className="flex-1 bg-(--surface-hover) border border-(--border) rounded-lg px-4 py-3 font-mono text-2xl text-center tracking-[0.3em] text-foreground">
                                        {showPin ? formatPin(generatedPin) : "••••-••••"}
                                    </div>
                                    <button onClick={() => setShowPin(!showPin)} className="p-3 text-(--text-muted) hover:text-(--accent-primary) hover:bg-(--accent-primary)/10 rounded-lg border border-(--border) transition-colors">
                                        {showPin ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                                    </button>
                                    <button onClick={copyPin} className="p-3 text-(--text-muted) hover:text-(--accent-primary) hover:bg-(--accent-primary)/10 rounded-lg border border-(--border) transition-colors">
                                        <Copy className="w-5 h-5" />
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-medium text-(--text-muted) uppercase tracking-wider">URL Akses</label>
                                <div className="flex items-center gap-2">
                                    <div className={`flex-1 border rounded-lg px-4 py-3 text-sm truncate flex items-center gap-2 ${generatedSessionType === "GATE" ? "bg-(--accent-primary)/10 border-indigo-200 text-(--accent-primary)" : "bg-emerald-50 border-emerald-200 text-emerald-700"}`}>
                                    ...
                                    <a 
                                        ...
                                        className={`p-3 rounded-lg border transition-colors ${generatedSessionType === "GATE" ? "text-indigo-500 hover:text-(--accent-primary) hover:bg-(--accent-primary)/10 border-indigo-200" : "text-emerald-500 hover:text-emerald-600 hover:bg-emerald-50 border-emerald-200"}`}
                                    >
                            ...
                            <div className="pt-4 border-t border-(--border) space-y-3">
                                <button
                                    ...
                                    className={`w-full py-3 text-white font-medium rounded-xl transition-colors flex items-center justify-center gap-2 ${generatedSessionType === "GATE" ? "bg-(--accent-primary) hover:opacity-90" : "bg-emerald-600 hover:bg-emerald-700"}`}
                                >
