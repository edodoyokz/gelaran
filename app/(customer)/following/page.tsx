...
export default function FollowingPage() {
    ...
    const handleUnfollow = async (organizerSlug: string, id: string, organizerName: string) => {
        if (pendingUnfollowId !== id) {
            setPendingUnfollowId(id);
            setStatusMessage(`Tekan sekali lagi untuk berhenti mengikuti ${organizerName}.`);
            return;
        }
        ...
        try {
            ...
            if (!data.success) {
                setStatusMessage(data.error?.message || "Gagal berhenti mengikuti organizer.");
                return;
            }

            setFollowing((prev) => prev.filter((f) => f.id !== id));
            setStatusMessage(`Kamu berhenti mengikuti ${organizerName}.`);
        } catch {
            setStatusMessage("Terjadi kesalahan saat berhenti mengikuti organizer.");
        } finally {
    ...
    const handleToggleNotifications = async (
        ...
        try {
            ...
            setStatusMessage(
                !currentNotify
                    ? `Notifikasi event baru untuk ${organizerSlug} diaktifkan.`
                    : `Notifikasi event baru untuk ${organizerSlug} dinonaktifkan.`,
            );
        } catch {
            setStatusMessage("Terjadi kesalahan saat mengupdate notifikasi.");
        } finally {
            setActionLoading(null);
        }
    };
                ...
                {following.length === 0 ? (
                    ...
                    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                        {filteredFollowing.map((item) => (
                            <article
                                key={item.id}
                                className="rounded-[1.9rem] border border-[rgba(1,89,89,0.08)] bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(246,250,249,0.95))] p-5 shadow-[0_18px_44px_rgba(1,89,89,0.07)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_24px_56px_rgba(1,89,89,0.11)] dark:border-[rgba(78,222,225,0.12)] dark:bg-[linear-gradient(180deg,rgba(26,26,36,0.96),rgba(18,30,31,0.92))]"
                            >
                                ...
                                <div className="mt-5 flex flex-col gap-3">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        ...
                                        >
                                            ...
                                            {pendingUnfollowId === item.id ? "Konfirmasi" : "Berhenti ikuti"}
                                        </button>
                                    </div>
                                </div>
                            </article>
