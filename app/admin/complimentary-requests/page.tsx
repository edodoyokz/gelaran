...
export default function AdminComplimentaryRequestsPage() {
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 py-6">
            <div className="mb-6">
                <Link href="/admin" className="inline-flex items-center gap-2 text-gray-600 hover:text-gray-900">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">Complimentary Requests</h1>
                    <p className="text-gray-600">Approve/reject request tiket complimentary dari promotor.</p>
                </div>
                <select
                    value={statusFilter}
            ...
            {isLoading ? (
                ...
                <div className="space-y-4">
                    {requests.map((request) => (
                        <div key={request.id} className="bg-white border rounded-xl p-5">
                            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                                <div>
                                    ...
                                    <p className="text-sm text-gray-600">Requested by: {request.requestedBy.name} ({request.requestedBy.email})</p>
                                    <p className="text-sm text-gray-600">Guest: {request.guestName || "-"} ({request.guestEmail || "-"})</p>
                                    <p className="text-sm text-gray-600 mt-1">Total tiket diminta: <span className="font-semibold">{request.requestedTotal}</span></p>
                                    <div className="mt-2 space-y-1">
                                        {request.items.map((item) => (
                                            <p key={item.id} className="text-sm text-gray-700">• {item.ticketType.name} x{item.quantity}</p>
                                        ))}
                                    </div>
                                </div>

                                <div className="lg:text-right min-w-[250px]">
                                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold mb-3 bg-gray-100 text-gray-700">
                                        {request.status}
                                    </div>
                                    <p className="text-xs text-gray-500 flex lg:justify-end items-center gap-1">
