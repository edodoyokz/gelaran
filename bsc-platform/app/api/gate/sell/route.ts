...
export async function POST(request: NextRequest) {
    try {
        ...
        const parameter = {
            ...
            callbacks: {
                finish: `${process.env.NEXT_PUBLIC_APP_URL}/gate/payment-success?booking=${bookingCode}`,
                error: `${process.env.NEXT_PUBLIC_APP_URL}/gate/payment-failed?booking=${bookingCode}`,
                pending: `${process.env.NEXT_PUBLIC_APP_URL}/gate/payment-pending?booking=${bookingCode}`,
            },
            expiry: {
                unit: "minutes",
                duration: 15,
